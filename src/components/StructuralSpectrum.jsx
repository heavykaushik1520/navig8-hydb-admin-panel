...
function StructuralSpectrum() {
  ...
  const fetchVisitorsByStructuralSpectrum = async (page, limit, option) => {
    try {
      ...
      const res = await fetch(
        `${API_URL}/api/form/filter-by-structural-spectrum?option=${encodeURIComponent(
          option
  ...
  useEffect(() => {
    fetchVisitorsByStructuralSpectrum(1, 10, selectedOption);
  }, [selectedOption]);

  const handlePageChange = (page) => {
    if (page < 1 || page > pagination.totalPages) return;
    fetchVisitorsByStructuralSpectrum(
      page,
      ...
      <div className="row">
        ...
        >
          ...
          <div className="list-group">
            {structuralSpectrumOptions.map((opt, idx) => (
              ...
              <table className="table table-striped table-bordered table-hover">
                <thead>
                  <tr>
                    ...
                    <th className="thead-bg">Structural Spectrum</th>
                    ...
                    visitors.map((v) => (
                      <tr key={v.id}>
                        ...
                        <td>
                          {(() => {
                            try {
                              const parsed = JSON.parse(v.structuralSpectrum);
                              return Array.isArray(parsed)
                                ? parsed.join(", ")
                                : parsed;
                            } catch {
                              return v.structuralSpectrum || "-";
                            }
                          })()}
                        </td>
...
export default StructuralSpectrum;
